...
# PySol imports
#from mfxutil import Struct, openURL # openURL should be replaced with webbrowser.open('http://eample.com') from the standard lib.
PACKAGE = "RsHtmlFromPysol" # replaces "from util import PACKAGE".

# Toolkit imports
from rstkutil import bind, unbind_destroy, loadImage
#from tkwidget import MfxDialog


class Struct:
    """
    dict-like data structure object.
    """
...
class tkHTMLParser(htmllib.HTMLParser):
    """
    Objects:
    * htmllib.HTMLParser init takes a formatter.AbstractFormatter object as only argument.
    * The AbstractFormatter takes a writer (tkHTMLWriter) as argument.
    * The tkHTMLwriter takes a text widget as argument, to which the output is written.

    As data is being fed to the parser, it goes through the formatter and is written by the writer object:
        data (html input) -> parser -> formatter -> writer -> output (text widget)
...
class tkHTMLViewer(object):
    """
    A simple html viewer for displaying local html files.
    Based on the pysol project's documentation viewer.
    Can be used as a reference implementation of the classes above.
    (See # Write the html to the text widget: # in the display() method)
    ...
    def display(self, url, add=1, relpath=1, xview=0, yview=0):
        ...
        # Do not attempt to parse anything but standard file protocol.
        # ftp: and http: protocols could possibly also be read using urllib
        for p in ("ftp:", "gopher:", "http:", "mailto:", "news:", "telnet:"):
            if string.find(url, p) != -1:
                webbrowser.open(url, new=20)
                return
        ...
        # Show the watch ("loading") cursor and prepare the text widget:
