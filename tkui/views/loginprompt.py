...
class LoginPrompt(Dialog):
    ...
    def __init__(self, parent, title = None, fieldvars = None, username=''):
        self.Initial_username = username # super's __init__ calls body, which depends on this.
        Dialog.__init__(self, parent, title, fieldvars)

    def body(self, master):
        if not self.Fieldvars:
            self.Fieldvars = OrderedDict(
                username = [tk.StringVar(value=self.Initial_username), 'Username', dict()],
                password = [tk.StringVar, 'Password', dict(show="*")]
                )
